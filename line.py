...
def calculate_depth_statistics_with_qc_avg(df):
    ...
    ic_values = [ic_col.iloc[200]]  # 存儲當前段的所有IC值
    qc_values = [qc_col.iloc[200]]  # 存儲當前段的所有qc值
    ...
    for i in range(201, len(df)):
        if type_col.iloc[i] != current_type and mark.iloc[i] != '*':
            ...
            average_qc = sum(qc_values) / len(qc_values)  # 計算該段土壤的平均qc
            ...
            ic_values = [ic_col.iloc[i]]  # 重置IC值列表
            qc_values = [qc_col.iloc[i]]  # 重置qc值列表
        else:
    ...
    average_qc = sum(qc_values) / len(qc_values)
    result.append([current_type, start_depth, end_depth, average_ic, average_qc])

    # 創建 DataFrame 保存結果
    depth_stats_df = pd.DataFrame(result, columns=['Type', 'Upper Depth', 'Lower Depth', 'Average IC', 'Average qc'])
    
    print("土壤類型深度統計和平均 IC 以及平均 qc（排除前200筆資料）：")
    print(depth_stats_df)

    # 將結果保存為 Excel 文件
    depth_stats_df.to_excel('soil_depth_statistics_with_ic_and_qc_avg_skipping_200.xlsx', index=False, engine='openpyxl')
